...
function export_dataframes(M, records, times, results_folder, experiment_name, subexperiment_name, col_names_1)#, col_names_2)
    ...
    #     Dimension=manifold_dimension(M),
        # Time_3=times[3],
        # Iterations_3=maximum(first.(records[3])),
        # Objective_3=minimum([r[2] for r in records[3]]),
    )
    return B1#, B2
end
...
for (n, m) in gr_dims
    ...
    p0 = exp(M, anchor, vectors[N+2]) #data[minimum(Tuple(findmax(dists)[2]))]
    ...
    # Optimization
    pgm_constant = proximal_gradient_method(M, f_gr, g_gr, grad_g_gr, prox_h_gr, p0; pgm_kwargs_constant(constant_stepsize)...)
    ...
    if benchmarking
        pgm_constant_bm = @benchmark proximal_gradient_method($M, $f_gr, $g_gr, $grad_g_gr, $prox_h_gr, $p0; $pgm_bm_kwargs_constant($constant_stepsize)...)
