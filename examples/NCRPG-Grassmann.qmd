...
pgm_bm_kwargs(initial_stepsize, β) = [
    :record => [:Iteration, :Cost, :Iterate],
    :return_state => true,
    :stepsize => ProximalGradientMethodBacktracking(;
        strategy=:nonconvex,
...
for (n, m) in gr_dims
    ...
    pgm = proximal_gradient_method(M, f_gr, g_gr, grad_g_gr, p0; prox_nonsmooth=prox_h_gr, pgm_kwargs(initial_stepsize, β)...)
    ...
    if benchmarking
        ...
        pgm_bm = @benchmark proximal_gradient_method($M, $f_gr, $g_gr, $grad_g_gr, $p0; prox_nonsmooth=$prox_h_gr, $pgm_bm_kwargs($initial_stepsize, $β)...)
...
#| code-fold: true
#| output: asis
benchmarking && pretty_table(A1_Gr, backend = :markdown, column_labels=col_names_1)
```
