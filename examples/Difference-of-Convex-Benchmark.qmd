...
blue = paul_tol["vibrantblue"]
cyan = paul_tol["vibrantcyan"]
orange = paul_tol["vibrantorange"]
magenta = paul_tol["vibrantmagenta"]
teal = paul_tol["vibrantteal"]
red = paul_tol["vibrantred"]
```

## The DC Problem

This Example is part of [BergmannFerreiraSantosSouza:2023](@ref), Section 7.1.
We want to compare how the two algorithms [Difference of Convex Algorithm (DCA)](https://manoptjl.org/stable/solvers/difference_of_convex/#Manopt.difference_of_convex_algorithm)
and [Difference of Convex Proximal Point Algorithm (DCPPA)](https://manoptjl.org/stable/solvers/difference_of_convex/#Manopt.difference_of_convex_proximal_point), cf. [SouzaOliveira:2015](@ref) scale with manifold dimension.
...
It needs a few more iterations and is hence slighlty slower. But it obtains the same cost
...
plot!(dims, dca_times; label="DCA",
    color=blue,
)
plot!(dims, dcppa_times; label="DCPPA", color=orange)
```
...
plot(;
    legend=:bottomright,
    xlabel="manifold dimension", ylabel="Iterations"
)
plot!(dims2, [values(dca_iterations)...]; label="DCA",color=blue)
plot!(dims2, [values(dcppa_iterations)...]; label="DCPPA", color=orange)
```

And for the developtment of the cost

```{julia}
fig = plot(;    legend=:topright, color=blue,
...
for (i,v) in enumerate(N2)
    plot!(
        fig,
        1:length(dca_costs[v]),
        abs.(dca_costs[v] .+ 1 / 4); # - f(p*)
    color=HSLA(HSL(blue),i/length(N)),
    label="DCA (d=$(dims2[i]))"
    )
end
for (i, v) in enumerate(N2)
    plot!(
        fig,
        1:length(dcppa_costs[v]),
        abs.(dcppa_costs[v] .+ 1 / 4); # - f(p*)
        color=HSLA(HSL(teal), i / length(N)),
