...
In this small Benchmark comparing the Difference of Convex Algprithm (DCA) [@BergmannFerreiraSantosSouza:2023]
and the Difference of Convex Proximal Point Algorithm (DCPPA) [@SouzaOliveira:2015] which solve
Difference of Convex (DC) problems of the form. This Benchmark reproduces the results from [@BergmannFerreiraSantosSouza:2023], Section 7.1.
...
plot!(dims, dcppa_times; label="DCPPA", color=teal)
```
...
plot!(dims2, [values(dca_iterations)...]; label="DCA",color=indigo)
plot!(dims2, [values(dcppa_iterations)...]; label="DCPPA", color=teal)
```
...
for (i,v) in enumerate(N2)
    ...
    color=HSLA(HSL(indigo),i/length(N)),
    label="DCA (d=$(dims2[i]))"
    )
end
for (i, v) in enumerate(N2)
