...
# The characteristic function for the set C is defined with 2eps() to avoid numerical issues
characteristic_C(M, p; op=[], radius=1) = (g(M, p; op=op, radius=radius) ≤ 2eps()) ? 0 : Inf;
...
function bench_aep(Manifold, center, radius, data)
    ...
    crpg_cn = proximal_gradient_method!(
        ...
        stopping_criterion=StopWhenGradientMappingNormLess(1e-7)|StopAfterIteration(5000),
    )
...
lines!(axis, n_range, [bi[:CRPG_CN][:time] for bi in b]; label="CRPG, constant step")
lines!(axis, n_range, [bi[:CRPG_BT][:time] for bi in b]; label="CRPG, backtracked step")
...
lines!(axis2, n_range, [bi[:CRPG_CN][:Iter] for bi in b]; label="CRPG constant step")
lines!(axis2, n_range, [bi[:CRPG_BT][:Iter] for bi in b]; label="CRPG backtracked step")
...
function plot_convergence(
    ...
    for i in 1:length(crpg_cn)
        ...
        # if write_csv
        #     CSV.write(
        #         "$(fn)convergence-$(dimensions[i]).csv",
        #         DataFrame(;
        #             Iterations=iterations_crpg_cn,
        #             RelativeErrors=relative_errors_crpg_cn,
        #             IterationsPGA=iterations_pga,
        #             RelativeErrorsPGA=relative_errors_pga,
        #         ),
        #     )
        # end
...
figs = plot_convergence(b, [2, 10, 100])
```
