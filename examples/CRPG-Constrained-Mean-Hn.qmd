...
function average_benchmarks(Manifold, center, radius, data; N=10)
  ...
  for i in 1:N
    # Perturb the center 
    n = length(center)
    starting_point = center + rand(n)
    push!(local_bench, bench_aep(Manifold, starting_point, radius, data))
  end

  stats = Dict(:CRPG_CN => Dict(), :CRPG_BT => Dict(), :PGA => Dict())

  stats[:CRPG_CN][:Iter] = mean(local_bench[:CRPG_CN][:Iter])
  stats[:CRPG_CN][:Cost] = mean(local_bench[:CRPG_CN][:Cost])
  stats[:CRPG_CN][:time] = mean(local_bench[:CRPG_CN][:time])

  stats[:CRPG_BT][:Iter] = mean(local_bench[:CRPG_BT][:Iter])
  stats[:CRPG_BT][:Cost] = mean(local_bench[:CRPG_BT][:Cost])
  stats[:CRPG_BT][:time] = mean(local_bench[:CRPG_BT][:time])

  stats[:PGA][:Iter] = mean(local_bench[:PGA][:Iter])
  stats[:PGA][:Cost] = mean(local_bench[:PGA][:Cost])
  stats[:PGA][:time] = mean(local_bench[:PGA][:time])
...
#| output: false
#| echo: false
b = [average_benchmarks(M, c, r, d) for (M, c, r, d) in zip(M_range, centers, radius_range, data)]
```
