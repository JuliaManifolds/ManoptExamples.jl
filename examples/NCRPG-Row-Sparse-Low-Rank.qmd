...
```math
h(X) = \μ \Vert X \Vert_{1, 2}
``` 
be the row sparsity-enforcing term given by the $\ell_{1,2}$-norm, where $\μ \ge 0$ is a regularization parameter.
...
```{julia}
#| output: false
function match_rows(Mr, X, sol, zero_rows; tol_zero = 1e-8, tol_nonzero = 1e-3)
    M, N, r = Manifolds.get_parameter(Mr.size)
    X_mat = embed(Mr, X)
    sol_mat = embed(Mr, sol)
    match_zero = 0
    match_nonzero = 0
    for j in 1:M 
        if j in zero_rows
            if norm(X.U[j, :] .* X.S) < tol_zero
                match_zero += 1
            end 
        else 
            if norm(X_mat[j, :] - sol_mat[j, :]) < tol_nonzero
                match_nonzero += 1
            end 
        end 
    end 
    return match_zero/length(zero_rows), match_nonzero/(M - length(zero_rows))
end 
#
...
```{julia}
#| output: false
function generate_data(M, N, r, s)
...
#| output: false
# NCRPG 
function prox_norm12(Mr::FixedRankMatrices, prox_param, X::SVDMPoint; c = c) 
    λ = prox_param * c
...
#
#RADMM 
function prox_norm12(Mr::FixedRankMatrices, prox_param, X1::Matrix{Float64}; c = c) 
    λ = prox_param * c
    M, N, k = Manifolds.get_parameter(Mr.size)
    Y1 = zeros(M, N)
    for i in 1:M
...
function f(M::FixedRankMatrices, X::SVDMPoint, A = A, c = c, y = y, max_cost = 1e5) 
    X_vec = vec(embed(M, X))
...
#
g(M::FixedRankMatrices, X::SVDMPoint, A = A, c = c, y = y) = 0.5*(norm(A*vec(embed(M, X)) - y))^2 
function grad_g(M::FixedRankMatrices, X::SVDMPoint, A = A, c = c, y = y) 
    X_mat = embed(M, X)
    return project(M, X, reshape(A'*(A*vec(X_mat) - y), size(X_mat)))
end 
h(M::FixedRankMatrices, X::SVDMPoint, A = A, c = c, y = y) = c * norm12(X)
```
...
for (r, m) in r_m_array
    ...
    #
    # Generate rank r matrix with s non-zero rows
    Sol_mat, A, y, zero_rows = generate_data(M, N, r, s)
    ...
    f(Mr, X) = f(Mr, X, A, c, y)
    g(Mr, X) = g(Mr, X, A, c, y)
    grad_g(Mr, X) = grad_g(Mr, X, A, c, y)
    h(Mr, X) = h(Mr, X, A, c, y)
    prox_norm12(Mr, prox_param, X) = prox_norm12(Mr, prox_param, X; c = c)
    #
    # Optimization
    # NCRPG
    ...
    # RADMM
    res_RADMM, succ, it_RADMM = RADMM_blind_deconv(A, M,N, r, c, prox_norm12; 
        max_iters  = max_iters , 
        start = start, 
        η = step_size,  
        stop = stop_RADMM
    )  
    bm_RADMM = @benchmark RADMM_blind_deconv($A, $M, $N, $r, $c, $prox_norm12; 
        max_iters  = max_iters , 
        start = $start, 
        η = $step_size,  
        stop = stop_RADMM
...
# | code-fold: true
header_1 = list
benchmarking && pretty_table(df_RADMM; backend = Val(:markdown), header = header_1)
benchmarking && pretty_table(df_NCRPG; backend = Val(:markdown), header = header_1)
benchmarking && pretty_table(df_NCRPG_bt; backend = Val(:markdown), header = header_1)
```
...
benchmarking && pretty_table(df_distances; backend = Val(:markdown), header = header_2)
```
