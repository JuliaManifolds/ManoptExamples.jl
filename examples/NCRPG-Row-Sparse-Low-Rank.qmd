...
#| output: false
# NCRPG 
function prox_norm12(Mr::FixedRankMatrices, prox_param, X::SVDMPoint; c = c, r=1) 
    λ = prox_param * c
...
function f(M::FixedRankMatrices, X::SVDMPoint, A, c, y, max_cost = 1e5) 
    X_vec = vec(embed(M, X))
...
#
g(M::FixedRankMatrices, X::SVDMPoint, A, c, y) = 0.5*(norm(A*vec(embed(M, X)) - y))^2 
function grad_g(M::FixedRankMatrices, X::SVDMPoint, A, c, y) 
    X_mat = embed(M, X)
    return project(M, X, reshape(A'*(A*vec(X_mat) - y), size(X_mat)))
end 
h(M::FixedRankMatrices, X::SVDMPoint, A, c, y) = c * norm12(X)
```
...
for (r, m) in r_m_array
    ...
    h(Mr, X) = h(Mr, X, A, c, y)
    prox_norm12(Mr, prox_param, X::SVDMPoint) = prox_norm12(Mr, prox_param, X; c = c, r=r)
