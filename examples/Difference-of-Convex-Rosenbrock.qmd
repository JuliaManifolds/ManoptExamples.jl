...
```{julia}
pEGD = gradient_descent(M, f, ∇f!!, p0;
...
```{julia}
check_gradient(M_rb, f, grad_f, p0, [0.0, 0.1]; plot=true)
```
...
```{julia}
check_gradient(M, g, ∇g, p0, [0.1, 0.1]; plot=true)
```


```{julia}
check_gradient(M, h, ∇h, p0, [0.1, 0.1]; plot=true)
```
