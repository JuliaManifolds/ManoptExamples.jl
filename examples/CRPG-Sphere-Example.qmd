...
function _grad_g(S, p, qs) 
    grad_g_val = zeros(length(p))
    for q_k in qs
        grad_g_val -= log(S, p, q_k)
    end
    return grad_g_val/N
end
...
function generate_data(S, radius)
    anchor = rand(S)
...
get_λ_δ(α_1, α_2, α_g, δ) = (sqrt(4 * (α_2 - α_1)^2 + 1/(2+δ)^2 * α_g^2) - 2 * (α_2 - α_1)) / (2 * α_g^2)
get_ζ_δ(δ) = pi/(2 + δ) * cot(pi/(2 + δ))
#
function get_const_stepsize(radius, τ)
    # α_1 ≤ h(p) ≤ α_2 ∀p ∈ B(anchor, radius)
    α_1 = 0.0
    α_2 = τ * 2 * radius
    # ||grad_g(p)|| ≤ α_g ∀p ∈ B(anchor, radius)
    α_g = 2 * radius 
    L_g = 1
    # Compute optimal δ for constant stepsize
    δ_grid = 0.0:0.001:0.4
    vals = [min(get_λ_δ(α_1, α_2, α_g, δ), get_ζ_δ(δ)/L_g) for δ in 0.0:0.001:0.4]
...
function get_initial_stepsize(radius, τ, δ = 1e-8)
    ...
    # ||grad_g(p)|| ≤ α_g ∀p ∈ B(anchor, radius)
    α_g = 2 * radius 
    # Compute initial guess s for backtracking: we need s < λ_δ
    s = get_λ_δ(α_1, α_2, α_g, δ)
    return s 
end 
...
# | code-fold: true
# | output: asis
benchmarking && pretty_table(A1_SPD; backend = :markdown, column_labels = col_names_1)
```
