...
grad_g(M, p) = ManifoldDiff.grad_distance(M, noise, p)
prox_h(M, λ, p) = ManoptExamples.prox_Total_Variation(M, α * λ, p)
```
...
pgm_bm_kwargs = [
    :λ => i->1/i,
    :stopping_criterion => StopWhenAny(StopAfterIteration(max_iters), Manopt.StopWhenGradientMappingNormLess(atol)),
]
```
...
#
pgm = proximal_gradient_method(Hn, f, grad_g, prox_h, noise; pgm_kwargs...)
...
if benchmarking
    ...
    #
    experiments = ["CPPA", "PGM"]#["RCBM", "PBA", "SGM", "CPPA", "PGM"]
    records = [cppa_record, pgm_record]#[rcbm_record, pba_record, sgm_record, cppa_record, pgm_record]
    results = [cppa_result, pgm_result]#[rcbm_result, pba_result, sgm_result, cppa_result, pgm_result]
    times = [
    ...
    if export_table
        for (time, record, result, experiment) in zip(times, records, results, experiments)
            A = cat(first.(record), [r[2]/length(noise) for r in record]; dims=2)
            CSV.write(
...
if export_result
    ...
    ball_pgm = convert.(PoincareBallPoint, pgm_result)
    #
    # Plot results
    plot!(
        ball_scene,
    ...
    matrix_pgm = matrixify_Poincare_ball(ball_pgm)
    CSV.write(
        joinpath(results_folder, experiment_name * "-pgm_optimum.csv"),
        DataFrame(matrix_pgm, :auto);
        header=["x", "y"],
    )
    #
