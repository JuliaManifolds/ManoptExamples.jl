...
dims = [2, 10, 100]#, 200] #[(10, 5), (100, 5), (200, 5)]
μs = [0.1, 0.5, 1.0]
...
h(M, p, μ) = μ * norm(p, 1)
prox_h(M, λ, p, μ) = prox_l1_Hn(M, λ * μ, p) # hcat([prox_l1_Hn(M.manifold, λ * μ, p[:, i]) for i in 1:M.size[1]]...)
# 
...
for n in dims
    ...
    # Define manifold
    M = Hyperbolic(n) #PowerManifold(Hyperbolic(n - 1), ArrayPowerRepresentation(), m)#
    ...
    for test in 1:n_tests
        ...
        for (c, μ) in enumerate(μs)
            ...
            contraction_factor = 0.9 #0.999999
            ...
            #
            # Collect sparsities
            sparsity_pgm_cn = sum(abs.(pgm_result_cn) .< 1e-8)/n
            sparsity_pgm_bt = sum(abs.(pgm_result_bt) .< 1e-8)/n
            # sparsity_cppa = sum(abs.(cppa_result) .< 1e-8)/n
...
CSV.write(joinpath(results_folder, "results-Hnm-time-iter-$(n_tests)-$(dims[end]).csv"), df_results_time_iter)
CSV.write(joinpath(results_folder, "results-Hnm-obj-spar-$(n_tests)-$(dims[end]).csv"), df_results_obj_spar)
```
