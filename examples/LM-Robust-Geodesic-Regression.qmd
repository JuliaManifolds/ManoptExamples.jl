...
We first run the ``[LevenbergMarquardt](@extref `Manopt.LevenbergMarquardt`)``{=commonmark} solver without
...
As comparison we take a scaled version of the ``[`HuberRobustifier`](@extref `Manopt.HuberRobustifier`)``{=commonmark}
by ``[concatenting](@extref `Manopt.ComposedRobustifierFunction`)``{=commonmark} it with a scaling,
so that we move the “cutoff” to a value `1.0e-4`, see ``[`ScaledRobustifierFunction`](@extref `Manopt.ScaledRobustifierFunction`)``{=commonmark}.
