...
```{julia}
cmb_kwargs(diameter, domf, k_max) = [
    diameter => diameter,
    domain => domf,
    k_max => k_max,
...
Before running the experiments, we initialize data collection tables that we will use later
```{julia}
A1 = DataFrame(;
        a="Dimension",
        b="Iterations",
...
for n in hn_sn_dims
    ...
    f_hn(M, p) = f(M, p, data)
    domf_hn(M, p) = domf(M, p, p0, diameter)
...
for n in spd_dims
    ...
    domf_spd(M, p) = domf(M, p, p0, diameter)
...
for n in hn_sn_dims
    ...
    p0 = data[minimum(Tuple(findmax(dists)[2]))]
    f_sn(M, p) = f(M, p, data)
    domf_sn(M, p) = domf(M, p, p0, diameter)
