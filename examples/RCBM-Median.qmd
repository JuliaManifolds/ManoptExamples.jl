...
for n in hn_sn_dims
    ...
    diameter_hn = 2 * maximum(dists) + 1e-1
    centroid_hn = data[minimum(Tuple(findmax(dists)[2]))]
    p0 = data[1]

    f_hn(M, p) = f(M, p, data)
    domf_hn(M, p) = domf(M, p, centroid_hn, diameter_hn)
...
for n in spd_dims
    ...
    data = [rand(M) for _ in 1:N]
    dists = [distance(M, z, y) for z in data, y in data]
    diameter_spd = 2 * maximum(dists) + 1e-1
    centroid_spd = data[minimum(Tuple(findmax(dists)[2]))]
    p0 = data[1]

    f_spd(M, p) = f(M, p, data)
    domf_spd(M, p) = domf(M, p, centroid_spd, diameter_spd)
...
for n in hn_sn_dims
    ...
    push!(north, 1.0)
    data = [close_point(M, north, diameter_sn / 2) for _ in 1:n]
    dists = [distance(M, z, y) for z in data, y in data]
