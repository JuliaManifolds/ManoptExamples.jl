...
data = noise
diameter = 100/n * maximum([distance(H, data[i], data[j]) for i in 1:n, j in 1:n])
...
domf(M, p) = distance(M, p, data) < diameter / 2 ? true : false
function ∂f(M, p)
    return 1 / length(data) * (
...
```{julia}
#| output: false
rcbm = convex_bundle_method(Hn, f, ∂f, data; rcbm_kwargs...)
rcbm_result = get_solver_result(rcbm)
rcbm_record = get_record(rcbm)
#
pba = proximal_bundle_method(Hn, f, ∂f, data; pba_kwargs...)
pba_result = get_solver_result(pba)
pba_record = get_record(pba)
#
sgm = subgradient_method(Hn, f, ∂f, data; sgm_kwargs...)
sgm_result = get_solver_result(sgm)
sgm_record = get_record(sgm)
#
cppa = cyclic_proximal_point(Hn, f, proxes, data; cppa_kwargs...)
...
#| output: false
if benchmarking
    pba_bm = @benchmark proximal_bundle_method($Hn, $f, $∂f, $data; $pba_bm_kwargs...)
    rcbm_bm = @benchmark convex_bundle_method($Hn, $f, $∂f, $data; $rcbm_bm_kwargs...)
    sgm_bm = @benchmark subgradient_method($Hn, $f, $∂f, $data; $sgm_bm_kwargs...)
    cppa_bm = @benchmark cyclic_proximal_point($Hn, $f, $proxes, $data; $cppa_bm_kwargs...)
    #
