...
p0 = noise
diam = 2.5 * distance(Hn, p0, data) # Diameter for the convex bundle method
```
...
function ∂f(M, p)
    return 1 / length(data) *
        (grad_distance(M, data, p) + α * ManoptExamples.subgrad_TV(M, p; atol=atol))
end
proxes = ((M, λ, p) -> prox_distance(M, λ, data, p, 2), (M, λ, p) -> prox_TV(M, α * λ, p))
```
...
c = cyclic_proximal_point(
    ...
    proxes,
    p0;
    stopping_criterion=StopWhenAny(StopAfterIteration(max_iters),StopWhenChangeLess(10.0^-8)),
    debug=[
        :Iteration,
        " | ",
...
if export_result
    ...
    #
    # Suppress next plots for clarity
