...
function theoretical_lipschitz_constant(M, anchor, n, R=D, N=100_000)
    ...
    for i in 1:N
        ...
        push!(constants, 12n * log(det(p))^2)
    end
    return maximum(constants)
end
```
...
total_records = []
column_names = ["Iterations", "Objective"]
...
# | code-fold: true
# | output: asis
ENV["DATAFRAMES_COLUMNS"] = 0
# benchmarking && pretty_table(A1_SPD; tf = tf_markdown, header = col_names_1, max_num_of_columns=0)

struct DataFrameWrapper
    df
end

Base.show(io::IO, mime::MIME"text/plain", w::DataFrameWrapper) = show(io, mime, w.df)

# DataFrameWrapper(A1_SPD)
show(A1_SPD)
# show(DataFrameWrapper(pretty_table(A1_SPD; tf = tf_markdown, header = col_names_1)))
# DataFrameWrapper(A1_SPD)
# pretty_table(A1_SPD; tf = tf_markdown, header = col_names_1, limit_printing=false, max_num_of_columns=-1)
```
